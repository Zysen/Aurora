	...
	stats.WIDGETS.MemoryUsage = function(instanceId, data, purgeData) {
		...
		return {
			...
			load : function() {
				...
				DATA.getChannelE(instanceId, stats.CHANNEL_ID, stats.CHANNELS.memory_usage).mapE(function(memoryUsage) {
					memoryUsage = JSON.parse(binary.arrayBufferToString(memoryUsage));
	...
	stats.WIDGETS.MemoryUsageHistory = function(instanceId, data, purgeData) {
		...
		return {
			...
			load : function() {
				...
				var chart = new CanvasJS.Chart(container.id, chartOptions);
				var memoryUsageE = DATA.getChannelE(instanceId, stats.CHANNEL_ID, stats.CHANNELS.memory_usage).blindE(50).mapE(function(memoryUsage) {
					memoryUsage = JSON.parse(binary.arrayBufferToString(memoryUsage));
					var used = memoryUsage.total - memoryUsage.free;
